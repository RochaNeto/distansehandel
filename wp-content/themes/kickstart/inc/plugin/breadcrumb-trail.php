	...
	function mnky_breadcrumb_get_items( $args ) {
		...
		/* If viewing any type of archive. */
			...
			if ( is_category() || is_tag() || is_tax() ) {
				...
				if ( is_day() )
					$item['last'] = __( 'Archives for ', 'breadcrumbs-plus' ) . get_the_time( 'F j, Y' );

				elseif ( is_month() )
					$item['last'] = __( 'Archives for ', 'breadcrumbs-plus' ) . single_month_title( ' ', false );

				elseif ( is_year() )
					$item['last'] = __( 'Archives for ', 'breadcrumbs-plus' ) . get_the_time( 'Y' );
			}

			else if ( is_author() )
				$item['last'] = __( 'Archives by: ', 'breadcrumbs-plus' ) . get_the_author_meta( 'display_name', $wp_query->post->post_author );
		}

		/* If viewing search results. */
		else if ( is_search() )
			$item['last'] = __( 'Search results for "', 'breadcrumbs-plus' ) . stripslashes( strip_tags( get_search_query() ) ) . '"';
