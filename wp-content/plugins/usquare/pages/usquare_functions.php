...
function generate_usquare(usquareAdmin &$usquare_this, $settings, &$itemsArray, $plugin_url, $id, $icons, $mode=0)
{
	...
	if ($mode==0 || $mode==1) {
		...
		if (count($fonts_to_load)) {
			$frontHtml.='<link rel="stylesheet" type="text/css" href="http://fonts.googleapis.com/css?family=';
			$foreach_i=0;
...
if (count($itemsArray)>0) {
	foreach ($itemsArray as $key => $arr) {
		...
		$image='<img src="'.$arr['item-image'].'" class="usquare_square '.$img_class.'" '.$img_style.' alt="" />';
		
		if ($settings['grayscale']==1) {
			$gray_image_class='';
			if ($arr['item-image-position']==1) $gray_image_class=' right_gray_usquare_square';
			$image.='<img src="'.$arr['item-image-gray'].'" style="position: absolute;" class="usquare_square '.$img_class.$gray_image_class.'" '.$img_style.' alt="" />';
			//echo '<pre>'; print_r($arr); echo '<pre>'; exit;
		}
		...
		if ($arr['item-link-image']==1) {
			...
			if ($arr['item-link-image-opened']==0) {
				$image='<a '.$target.' href="'.$arr['item-link-image-url'].'" rel="'.$arr['item-link-image-rel'].'">'.$image.'</a>';
			} else {
				$image='<a '.$data_target.' data-href="'.$arr['item-link-image-url'].'" data-rel="'.$arr['item-link-image-rel'].'" data-only-open="1">'.$image.'</a>';
			}
		}
...
$frontHtml.='
				<div class="usquare_block_extended" style="'.$item_style.'">
					<a class="close"><img src="'.$plugin_url.'images/close.png" alt="close"/></a>
...
{
	$google_fonts_ok=0;
	if (isset($settings['google_fonts'])) if (strlen($settings['google_fonts'])>1000) {
		$google_fonts_ok=1;
		$fonts=$settings['google_fonts'];
	}
	if ($google_fonts_ok==0) $fonts=@file_get_contents($plugin_path.'/fonts/fonts.txt');
...
/*
	...
	static public function makethumb_image_db ($url, $w, $h, $opt=array(), $suffix = '', $dest_path = '', &$return_array2=NULL) {
		...
		$thmb = $wpdb->get_row('SELECT * FROM ' . $table . ' WHERE ukey="'.mysql_real_escape_string($ukey).'"', ARRAY_A);
		if ($thmb!==NULL) {
			if (!is_file($thmb['dest_file']))
			{
				$wpdb->query('DELETE FROM ' . $table . ' WHERE ukey="'.mysql_real_escape_string($ukey).'"');
				$thmb=NULL;
			}
		}
	...
	static public function makethumb_image($url, $w, $h, $opt=array(), $suffix = '', $dest_path = '', &$return_array=NULL) {
		...
		$predicted_file=usquare_image_class::predict_final_file($file, $w, $h, $opt, $suffix, $dest_path);
