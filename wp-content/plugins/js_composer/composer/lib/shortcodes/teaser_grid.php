    ...
    protected function content( $atts, $content = null ) {
        ...
        while ( $my_query->have_posts() ) {
            ...
            if ( $grid_layout == 'title_thumbnail_text' ) {
                if ( $post_title ) 	{
                    $to_filter = '<h5 class="post-title">' . $link_title_start . $post_title . $link_title_end . '</h5>';
                ...
                if ( $thumbnail ) {
                    $to_filter = '<div class="post-thumb portfolio_item">' . $link_image_start . $thumbnail . $link_image_end .'</div>';
                ...
                if ( $content ) {
                    $to_filter = '<div class="entry-content">' .get_post_meta($post->ID, 'port-descr', 1). '</div>';
            ...
            // If grid layout is: Thumbnail + Title + Text
                ...
                if ( $thumbnail ) {
                    $to_filter = '<div class="post-thumb portfolio_item"> <div class="view view-first">' . $link_image_start . $thumbnail . $link_image_end .'<div class="mask"><a href="' .$p_img_large[0]. '" rel="prettyPhoto" class="info"></a><a href="'.get_permalink().'" class="link"></a></div></div></div>';
                    $teasers .= apply_filters('vc_teaser_grid_thumbnail', $to_filter, array("grid_layout" => $grid_layout, "ID" => $post_id, "thumbnail" => $thumbnail, "media_link" => $p_link) );
                }
                if ( $post_title ) 	{
                    $to_filter = '<div class="descr"><h5 class="post-title">' . $link_title_start . $post_title . $link_title_end . '</h5>';
                    $teasers .= apply_filters('vc_teaser_grid_title', $to_filter, array("grid_layout" => $grid_layout, "ID" => $post_id, "title" => $post_title, "media_link" => $p_link) );
                }
                if ( $content ) {
                    $to_filter = '<div class="entry-content">' .get_post_meta($post->ID, 'port-descr', 1). '</div></div>';
                    $teasers .= apply_filters('vc_teaser_grid_content', $to_filter, array("grid_layout" => $grid_layout, "ID" => $post_id, "content" => $content, "media_link" => $p_link) );
                }
            }
            ...
            // If grid layout is: Thumbnail + Title
                ...
                if ( $thumbnail ) {
                    $to_filter = '</div><div class="post-thumb portfolio_item">' . $link_image_start . $thumbnail . $link_image_end .'</div>';
                    $teasers .= apply_filters('vc_teaser_grid_thumbnail', $to_filter, array("grid_layout" => $grid_layout, "ID" => $post_id, "thumbnail" => $thumbnail, "media_link" => $p_link) );
                }
                if ( $post_title ) 	{
                    $to_filter = '<h5 class="post-title">' . $link_title_start . $post_title . $link_title_end . '</h5></div>';
                    $teasers .= apply_filters('vc_teaser_grid_title', $to_filter, array("grid_layout" => $grid_layout, "ID" => $post_id, "title" => $post_title, "media_link" => $p_link) );
                }
            }
        ...
        if ( $grid_template == 'carousel' ) {
            $output .= apply_filters( 'vc_teaser_grid_carousel_arrows', '<a href="#" class="prev"></a> <a href="#" class="next"></a>' );
        }
