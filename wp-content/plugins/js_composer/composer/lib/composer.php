...
class WPBakeryVisualComposer extends WPBakeryVisualComposerAbstract {
   ...
   ---------------------------------------------------------- */
    ...
    public function elementBackendHtmlJavascript_callback() {
        global $current_user;
        get_currentuserinfo();
        $b_data_element = $this->post( 'data_element' );

        /** @var $settings - get use group access rules */
        $settings = WPBakeryVisualComposerSettings::get('groups_access_rules');
        $role = $current_user->roles[0];

        if ( $b_data_element == 'vc_column' && $this->post( 'data_width' )!==null ) {
            $output = do_shortcode( '[vc_column width="'. $this->post( 'data_width' ).'"]' );
            echo $output;
        } elseif($b_data_element == 'vc_row' || $b_data_element == 'vc_row_inner') {
            $output = do_shortcode( '['.$b_data_element.']' );
            echo $output;
        } elseif ( !isset($settings[$role]['shortcodes']) || ( isset($settings[$role]['shortcodes'][$b_data_element]) && (int)$settings[$role]['shortcodes'][$b_data_element] == 1 ) ) {
            $output = do_shortcode( '['.$b_data_element.']' );
            echo $output;
        }
        die();
    ...
    public function elementRowBackendHtmlJavascript_callback() {
        ...
        get_currentuserinfo();
        $b_data_element = $this->post( 'data_element' );
        $b_data_width = $this->post( 'data_width' );
        /** @var $settings - get use group access rules */
        $settings = WPBakeryVisualComposerSettings::get('groups_access_rules');
        $role = $current_user->roles[0];
        if ( $b_data_element == 'vc_column' ) {
            $output = do_shortcode( '[vc_row][vc_column width="'. ($b_data_width ? $b_data_width : '1/1') .'"][/vc_row]' );
            echo $output;
        } elseif( !isset($settings[$role]['shortcodes']) || ( isset($settings[$role]['shortcodes'][$b_data_element]) && (int)$settings[$role]['shortcodes'][$b_data_element] == 1 ) ) {
            $output = do_shortcode( '[vc_row][vc_column width="1/1"]['.$b_data_element.'][/vc_column][/vc_row]' );
            echo $output;
        }
        die();
    }
